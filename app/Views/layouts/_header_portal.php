<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php echo isset($pageTitle) ? htmlspecialchars($pageTitle) : 'SFMS Portal'; ?> - SFMS</title>

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <link href="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css" rel="stylesheet" />
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/select2-bootstrap-5-theme@1.3.0/dist/select2-bootstrap-5-theme.min.css" />

    <style>
        /* Basic Portal Styles */
        body { background-color: #e9ecef; /* Light background for portal */ }
        .portal-container { max-width: 1140px; margin: 20px auto; }
        .navbar { margin-bottom: 20px; box-shadow: 0 2px 4px rgba(0,0,0,.1);}
        .card { margin-bottom: 20px; }
        .flash-message { margin-top: 15px; margin-bottom: 15px; } /* Added margin-bottom */
        .error { color: #721c24; background-color: #f8d7da; border-color: #f5c6cb; padding: .75rem 1.25rem; margin-bottom: 1rem; border: 1px solid transparent; border-radius: .25rem; }
        .success { color: #155724; background-color: #d4edda; border-color: #c3e6cb; padding: .75rem 1.25rem; margin-bottom: 1rem; border: 1px solid transparent; border-radius: .25rem;}
         /* Select2 height adjustment */
        .select2-container .select2-selection--single { height: calc(1.5em + .75rem + 2px); }
        .select2-container--bootstrap-5 .select2-selection--single .select2-selection__rendered { line-height: 1.5; padding-left: .75rem; }
        .select2-container--bootstrap-5 .select2-selection--single .select2-selection__arrow { height: calc(1.5em + .75rem); }
        /* Ensure filter form elements align */
         .filter-form label { margin-bottom: 0.1rem; font-size: 0.9em;}
         .filter-form .form-select-sm { padding-top: 0.4rem; padding-bottom: 0.4rem; }

    </style>
</head>
<body class="d-flex flex-column min-vh-100">

<nav class="navbar navbar-expand-lg navbar-light bg-white border-bottom shadow-sm"> <?php // Light navbar for portal ?>
  <div class="container-fluid">
    <a class="navbar-brand" href="/sfms_project/public/portal/dashboard">SFMS Portal</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#portalNavbar" aria-controls="portalNavbar" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="portalNavbar">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item">
           <a class="nav-link <?php echo (strpos($_SERVER['REQUEST_URI'], '/portal/dashboard') !== false) ? 'active' : ''; ?>" href="/sfms_project/public/portal/dashboard"><i class="bi bi-house-door-fill"></i> Dashboard</a>
        </li>
        <li class="nav-item">
           <a class="nav-link <?php echo (strpos($_SERVER['REQUEST_URI'], '/portal/fees') !== false || strpos($_SERVER['REQUEST_URI'], '/portal/invoices') !== false || strpos($_SERVER['REQUEST_URI'], '/portal/payments') !== false) ? 'active' : ''; ?>" href="/sfms_project/public/portal/fees"><i class="bi bi-receipt"></i> My Fees</a>
        </li>
         <li class="nav-item">
           <a class="nav-link <?php echo (strpos($_SERVER['REQUEST_URI'], '/portal/profile') !== false) ? 'active' : ''; ?>" href="/sfms_project/public/portal/profile"><i class="bi bi-person-circle"></i> My Profile</a>
        </li>
      </ul>
      <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
          <li class="nav-item">
             <span class="navbar-text me-2">
                 Hello, <?php echo htmlspecialchars($_SESSION['username'] ?? 'User'); ?>!
             </span>
          </li>
         <li class="nav-item">
             <a class="nav-link" href="/sfms_project/public/logout"><i class="bi bi-box-arrow-right"></i> Logout</a>
         </li>
      </ul>
    </div>
  </div>
</nav>

<main class="container portal-container flex-shrink-0 mb-4"> <?php // Added mb-4 ?>

    <?php $flash_success = $_SESSION['flash_success'] ?? null; unset($_SESSION['flash_success']); ?>
    <?php $flash_error = $_SESSION['flash_error'] ?? null; unset($_SESSION['flash_error']); ?>
    <?php if ($flash_error): ?><div class="alert alert-danger flash-message"><?php echo $flash_error; ?></div><?php endif; ?>
    <?php if ($flash_success): ?><div class="alert alert-success flash-message"><?php echo $flash_success; ?></div><?php endif; ?>
